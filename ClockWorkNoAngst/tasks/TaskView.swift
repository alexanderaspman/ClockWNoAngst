        ...
        VStack{
            ...
            if  toggleShowCreateTask {
                ...
                HStack(alignment: .bottom) {
                    NavigationLink( destination:{ AddTaskRow(toggleShowCreateTask: $toggleShowCreateTask).environmentObject(realmManager)}
                                    ,  label: {Text("+")}).padding(.bottom,20).animatableFonts(size: 39, weight: .bold, design: .rounded).frame(maxWidth: .infinity).background(Color(colorBtn)).foregroundColor(.white).frame(width: 55,height: 55,alignment: .top).cornerRadius(45).frame(maxWidth:.infinity,alignment: Alignment(horizontal: .trailing, vertical: .bottom)).padding().background( Color(color1))
